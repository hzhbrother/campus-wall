...
| 版本 | 状态 | 说明 |
|---|---|---|
| **V1.1.0.3** | ✅ 最新 | 认证识图+头像提取、资质认证、宫格布局、图片限制3张、首页加载优化 |
| V1.1.0.1 | ⏹️ 上一版 | 角色管理批量删除、细节优化与稳定性提升 |
| V1.1.0 | ⏹️ 历史版本 | 核心功能集成熟可用 |
...
本次版本重点优化了认证审核流程、首页加载性能、个人中心体验，并新增资质认证功能。

#### 🆕 新增功能

- **认证审核识图+头像提取**：AI 识别模板中"头像"字段后自动标记头像区域，复审页面用 Canvas 从学生卡照片裁剪头像并展示，通过认证时可自动设为用户头像。
- **资质认证**：新增组织身份认证（学生会/广播站/团委/社团联合会等），管理员可在用户编辑中设置资质类型和验证状态，个人中心显示资质徽章。
- **个人中心宫格布局**：管理员菜单从列表式改为 4 列宫格布局，图标+文字一目了然。
- **个人中心背景图**：默认渐变改为 indigo→blue→cyan，管理员可在站点设置中配置自定义背景图 URL。
- **管理员可修改用户头像/邮箱/手机号**：用户编辑弹窗新增邮箱、手机号输入框，支持修改用户头像。

#### 🐛 Bug 修复

- **修复首页长内容/大图导致加载卡死**：列表 API 返回完整 content（最长 5000 字）和完整 base64 图片（单张最大 3MB），10 条帖子的 payload 可能高达数百 MB。修复后列表 API 截断 content 到 200 字、不返回 base64 图片只返回图片数量，PostCard 改为显示"附图 N 张"提示。
- **修复 OCR 只识别数字**：降低 bbox 重叠阈值（0.15→0.05），增加正则兜底和字段名搜索兜底，确保按管理员模板字段顺序提取。
- **修复个人中心刷新页面自动退出登录**：401 处理改温和（不立即删 token 跳转，改为清除用户态让 UI 自然降级），usePageRefresh 新增 skipInitial 避免重复请求。
- **修复模板下载失败**：token key 从 'token' 改为 'cw_token'。

#### ✨ 优化

- **图片上传限制**：最多 3 张图片，超过 2MB 自动压缩（1280px / JPEG 0.7），不超过则保留原图。
- **通过认证自动填充档案**：识别到的姓名/学号/年级/班级/邮箱/手机号等字段，通过认证时自动同步到用户资料。
...
- 🖼️ 图片压缩：前端 Canvas 压缩（超过 2MB 才压缩，最大边 1280px，JPEG 0.7），最多 3 张
